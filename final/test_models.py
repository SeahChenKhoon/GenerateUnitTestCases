...
def test_user_info_creation(setup_db):
    user = UserInfo(
        first_name="John",
        last_name="Doe",
    ...
    setup_db.add(user)
    setup_db.commit()
    retrieved_user = setup_db.query(UserInfo).filter_by(email="john.doe@example.com").first()
...
class Projects(Base):
    ...
    problem_statement = Column(Text)  # Changed from JSONB to Text
