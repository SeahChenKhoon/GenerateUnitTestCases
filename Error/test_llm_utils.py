...
def test_initialise_prompt_success(mock_config_path, mock_agent, mock_prompt_structure, mock_config_values):
...
temp\temp.py:29: in test_initialise_prompt_success
    with patch("builtins.open", mock_open(read_data=mock_yaml_content)) as mock_file:
...
================================== FAILURES ===================================
_______________________ test_initialise_prompt_success ________________________
temp\temp.py:46: in test_initialise_prompt_success
    assert result == "This is a test prompt with a placeholder: value"
E   AssertionError: assert None == 'This is a test prompt with a placeholder: value'
---------------------------- Captured stdout call -----------------------------
descriptor 'group' for 're.Match' objects doesn't apply to a 'int' object: No configuration path to the prompt given.
=========================== short test summary info ===========================
...
def test_initialise_prompt_success():
    ...
    with patch("builtins.open", mock_open(read_data=mock_yaml_content)) as mock_file:
        with patch("yaml.load", return_value=mock_config_values):
            with patch("re.finditer", return_value=[re.Match("{$placeholder}", 0, 14)]):
                with patch("re.sub", return_value=mock_prompt_structure.replace("{$placeholder}", "value")):
                    result = initialise_prompt(mock_agent)
                    mock_file.assert_any_call(f"{mock_config_path}/{mock_agent}/config.yaml")
                    mock_file.assert_any_call(f"{mock_config_path}/{mock_agent}/prompt.txt", "r")
...
================================== FAILURES ===================================
_______________________ test_initialise_prompt_success ________________________
temp\temp.py:41: in test_initialise_prompt_success
    with patch("re.finditer", return_value=[re.Match("{$placeholder}", 0, 14)]):
...
temp\temp.py:28: in test_initialise_prompt_file_not_found
    with patch("builtins.open", side_effect=FileNotFoundError):
E   NameError: name 'patch' is not defined
=========================== short test summary info ===========================
...
def test_initialise_prompt_no_placeholder_match(mock_config_path, mock_agent, mock_prompt_structure):
    ...
    with patch("builtins.open", mock_open(read_data=mock_yaml_content)) as mock_file:
        with patch("yaml.load", return_value={}):
            ...
            mock_file.assert_any_call(f"{mock_config_path}/{mock_agent}/config.yaml")
            mock_file.assert_any_call(f"{mock_config_path}/{mock_agent}/prompt.txt", "r")
            assert result == mock_prompt_structure
...
F                                                                        [100%]
================================== FAILURES ===================================
_________________ test_initialise_prompt_no_placeholder_match _________________
temp\temp.py:29: in test_initialise_prompt_no_placeholder_match
    with patch("builtins.open", mock_open(read_data=mock_yaml_content)) as mock_file:
E   NameError: name 'patch' is not defined
=========================== short test summary info ===========================
FAILED temp/temp.py::test_initialise_prompt_no_placeholder_match - NameError:...
1 failed in 0.24s
TEST CASE 3 Retry 1
---------------
import os
...
def test_initialise_prompt_no_placeholder_match():
    ...
    with patch("builtins.open", mock_open(read_data=mock_yaml_content)) as mock_file:
        with patch("yaml.load", return_value={}):
            with patch("builtins.open", mock_open(read_data=mock_prompt_structure)) as mock_prompt_file:
                result = initialise_prompt(mock_agent)
                mock_file.assert_any_call(f"{mock_config_path}/{mock_agent}/config.yaml")
                mock_prompt_file.assert_any_call(f"{mock_config_path}/{mock_agent}/prompt.txt", "r")
                assert result == mock_prompt_structure
...
from unittest.mock import patch, mock_open
import yaml
from module_name import initialise_prompt  # Replace 'module_name' with the actual module name
...
def test_initialise_settings_success(mock_config_path, mock_agent, mock_llm_settings):
    mock_yaml_content = yaml.dump(mock_llm_settings)
    with patch("builtins.open", mock_open(read_data=mock_yaml_content)) as mock_file:
...
temp\temp.py:29: in test_initialise_settings_success
    with patch("builtins.open", mock_open(read_data=mock_yaml_content)) as mock_file:
E   NameError: name 'patch' is not defined
=========================== short test summary info ===========================
...
def test_initialise_settings_success():
    ...
    mock_llm_settings = {"key": "value"}  # Replace with actual expected settings
...
temp\temp.py:28: in test_initialise_settings_file_not_found
    with patch("builtins.open", side_effect=FileNotFoundError):
E   NameError: name 'patch' is not defined
=========================== short test summary info ===========================
